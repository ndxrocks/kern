...
from kern.memory.strategies import MemoryOptimizationStrategy
...
from kern.utils.dttm import now_epoch_s
...
from kern.utils.string import parse_response_model_str
...
class MemoryManager:
    ...
    def get_model(self) -> Model:
        if self.model is None:
            try:
                ...
                log_error(e)
                log_error(
                    "Kern uses `openai` as the default model provider. Please provide a `model` or install `openai`."
                )
                exit(1)
    ...
    def _get_db_tools(
        ...
        def add_memory(memory: str, topics: Optional[List[str]] = None) -> str:
            ...
            from kern.db.base import UserMemory
