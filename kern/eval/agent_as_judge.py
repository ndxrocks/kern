...
from kern.db.schemas.evals import EvalType
from kern.eval.base import BaseEval
from kern.eval.utils import async_log_eval, log_eval_run, store_result_in_file
from kern.exceptions import EvalError
from kern.models.base import Model
from kern.run.agent import RunInput, RunOutput
from kern.run.team import TeamRunInput, TeamRunOutput
from kern.utils.log import log_warning, logger, set_log_level_to_debug, set_log_level_to_info
...
class AgentAsJudgeEval(BaseEval):
    ...
    debug_mode: bool = getenv("KERN_DEBUG", "false").lower() == "true"
    ...
    def get_evaluator_agent(self) -> Agent:
        ...
        if model is None:
            try:
                ...
                logger.exception(e)
                raise EvalError(
                    "Kern uses `openai` as the default model provider. Please run `pip install openai` to use the default evaluator."
                )
    ...
    def run(
        ...
        *,
        ...
        if self.telemetry:
            from kern.api.evals import EvalRunCreate, create_eval_run_telemetry
    ...
    async def arun(
        ...
        *,
        ...
        if self.telemetry:
            from kern.api.evals import EvalRunCreate, async_create_eval_run_telemetry
