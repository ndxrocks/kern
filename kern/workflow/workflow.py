...
if TYPE_CHECKING:
    from kern.os.managers import WebSocketHandler

from kern.agent.agent import Agent
from kern.db.base import AsyncBaseDb, BaseDb, ComponentType, SessionType
from kern.db.utils import db_from_dict
from kern.exceptions import InputCheckError, OutputCheckError, RunCancelledException
from kern.media import Audio, File, Image, Video
from kern.models.message import Message
from kern.models.metrics import RunMetrics, SessionMetrics
from kern.registry import Registry
from kern.run import RunContext, RunStatus
from kern.run.agent import RunContentEvent, RunEvent, RunOutput
...
from kern.run.team import TeamRunEvent
...
from kern.session.workflow import WorkflowChatInteraction, WorkflowSession
from kern.team.team import Team
from kern.utils.agent import validate_input
...
from kern.utils.print_response.workflow import (
    aprint_response,
    aprint_response_stream,
    print_response,
    print_response_stream,
)
from kern.utils.string import generate_id_from_name
...
from kern.workflow.types import (
    StepInput,
    StepMetrics,
    StepOutput,
    StepRequirement,
...
from kern.workflow.utils import (
...
class Workflow:
    ...
    def _resolve_run_params(
        ...
        *,
        ...
        """
        from kern.utils.merge_dict import merge_dictionaries
    ...
    def _handle_event(
        ...
        from kern.run.base import BaseRunOutputEvent
        ...
        if buffer_run_id:
            try:
                from kern.os.managers import event_buffer
        ...
        if buffer_run_id and websocket_handler:
            try:
                ...
                from kern.os.managers import websocket_manager
                ...
                if loop:
                    ...
                    from kern.utils.serialize import json_serializer
    ...
    def _set_debug(self) -> None:
        ...
        if self.debug_mode or getenv("KERN_DEBUG", "false").lower() == "true":
            use_workflow_logger()
            debug_level: Literal[1, 2] = (
                cast(Literal[1, 2], int(env)) if (env := getenv("KERN_DEBUG_LEVEL")) in ("1", "2") else self.debug_level
            )
    ...
    def _set_telemetry(self) -> None:
        ...
        telemetry_env = getenv("KERN_TELEMETRY")
        ...
        **kwargs: Any,
        ...
        if callable(self.steps):
            ...
            try:
                ...
                from kern.run.workflow import WorkflowErrorEvent
    ...
    def _run_workflow_agent_stream(
        ...
        **kwargs: Any,
        ...
        from kern.run.workflow import WorkflowCompletedEvent, WorkflowRunOutputEvent
        ...
        from kern.run.agent import RunContentEvent
        from kern.run.team import RunContentEvent as TeamRunContentEvent
        from kern.run.workflow import WorkflowAgentCompletedEvent, WorkflowAgentStartedEvent
    ...
    def _log_workflow_telemetry(self, session_id: str, run_id: Optional[str] = None) -> None:
        ...
        from kern.api.workflow import WorkflowRunCreate, create_workflow_run
    ...
    async def _alog_workflow_telemetry(self, session_id: str, run_id: Optional[str] = None) -> None:
        ...
        from kern.api.workflow import WorkflowRunCreate, acreate_workflow_run
    ...
    def deep_copy(self, *, update: Optional[Dict[str, Any]] = None) -> "Workflow":
        ...
        from kern.utils.log import log_debug, log_warning
        ...
        try:
            ...
            from kern.utils.log import log_error
