"""Migration utility to migrate your Kern tables from v1 to v2"""
...
from kern.db.migrations.utils import quote_db_identifier
from kern.db.schemas.memory import UserMemory
from kern.session import AgentSession, TeamSession, WorkflowSession
from kern.utils.log import log_error, log_info, log_warning
...
def get_table_content_in_batches(db: BaseDb, db_schema: str, table_name: str, batch_size: int = 5000):
    ...
    try:
        if type(db).__name__ == "MongoDb":
            from kern.db.mongo.mongo import MongoDb
            ...
            if type(db).__name__ == "PostgresDb":
                ...
                from kern.db.mysql.mysql import MySQLDb
