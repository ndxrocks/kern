...
from kern.db.schemas.evals import EvalRunRecord
...
def deserialize_record_id(record: dict, kern_field: str, surreal_field: Optional[str] = None) -> dict:
    if surreal_field is None:
        surreal_field = kern_field
    x = record.get(surreal_field)
    if isinstance(x, RecordID):
        record[kern_field] = x.id
        if kern_field != surreal_field:
