...
from kern.culture.manager import CultureManager
...
from kern.session import SessionSummaryManager
...
def set_debug(agent: Agent, debug_mode: Optional[bool] = None) -> None:
    ...
    debug_level: Literal[1, 2] = (
        cast(Literal[1, 2], int(env)) if (env := getenv("KERN_DEBUG_LEVEL")) in ("1", "2") else agent.debug_level
    )
    # If the default debug mode is set, or passed on run, or via environment variable, set the debug mode to True
    if agent.debug_mode or debug_mode or getenv("KERN_DEBUG", "false").lower() == "true":
...
def get_models(agent: Agent) -> None:
    from kern.metrics import ModelType
