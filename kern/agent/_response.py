...
from kern.models.fallback import acall_model_stream_with_fallback, call_model_stream_with_fallback
from kern.models.message import Message
from kern.models.response import ModelResponse, ModelResponseEvent
from kern.reasoning.step import NextAction, ReasoningStep, ReasoningSteps
from kern.run import RunContext
from kern.run.agent import Followups, RunEvent, RunOutput, RunOutputEvent
...
from kern.utils.reasoning import (
...
from kern.utils.string import parse_response_dict_str, parse_response_model_str
...
def handle_reasoning_event(
    ...
    """
    from kern.reasoning.manager import ReasoningEventType
...
def reason(
    ...
    """
    from kern.reasoning.manager import ReasoningConfig, ReasoningManager
...
def parse_response_with_parser_model(
    ...
    from kern.agent._messages import get_messages_for_parser_model
    ...
    if output_schema is not None:
        ...
        if run_response is not None:
            from kern.metrics import ModelType, accumulate_model_metrics
...
def parse_response_with_parser_model_stream(
    ...
    """Parse the model response using the parser model"""
    from kern.agent._messages import get_messages_for_parser_model_stream
...
def generate_response_with_output_model(
    ...
    from kern.agent._messages import get_messages_for_output_model
        ...
        if isinstance(output_schema, dict):
            if isinstance(run_response.content, str):
                ...
                if parsed_dict is not None:
                    ...
                    # Kern fallback: json_repair
                    from kern.repair import extract_json
                    parsed_dict = extract_json(run_response.content)
                    if parsed_dict is not None:
                        run_response.content = parsed_dict
                        if isinstance(run_response, RunOutput):
                            run_response.content_type = "dict"
            ...
            if isinstance(run_response.content, str) and agent.parse_response:
                ...
                try:
                    ...
                    if structured_output is not None:
                        ...
                        # Kern fallback: json_repair + LaTeX protection
                        from kern.repair import extract_json
                        parsed = extract_json(run_response.content)
                        if parsed is not None:
                            try:
                                structured_output = output_schema.model_validate(parsed)
                                run_response.content = structured_output
                                if isinstance(run_response, RunOutput):
                                    run_response.content_type = output_schema.__name__
                            except Exception:
                    ...
                    # Kern fallback: try json_repair on parse error
                    from kern.repair import extract_json
                    parsed = extract_json(run_response.content)
                    if parsed is not None:
                        try:
...
def handle_model_response_chunk(
    ...
    from kern.run.workflow import WorkflowRunOutputEvent
