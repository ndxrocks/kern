...
def get_tools(
    ...
    from kern.agent import _default_tools, _init
    ...
    for tool in tools:
        if isinstance(tool, Dict):
            ...
            try:
                ...
                _approval_type = getattr(tool, "_kern_approval_type", None)
...
def _maybe_create_audit_approval(
    ...
    if getattr(tool_execution, "approval_type", None) == "audit":
        from kern.run.approval import create_audit_approval
...
async def _amaybe_create_audit_approval(
    ...
    if getattr(tool_execution, "approval_type", None) == "audit":
        from kern.run.approval import acreate_audit_approval
