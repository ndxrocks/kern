...
if TYPE_CHECKING:
    from kern.compression.manager import CompressionManager
...
from kern.metrics import MessageMetrics, ModelType, ToolCallMetrics
from kern.models.message import Citations, Message
from kern.models.response import ModelResponse, ModelResponseEvent, ToolExecution
from kern.run.agent import CustomEvent, RunContentEvent, RunOutput, RunOutputEvent
from kern.run.requirement import RunRequirement
from kern.run.team import RunContentEvent as TeamRunContentEvent
from kern.run.team import TeamRunOutput, TeamRunOutputEvent
from kern.run.workflow import WorkflowRunOutputEvent
from kern.tools.function import (
    Function,
    FunctionCall,
...
from kern.utils.tools import get_function_call_for_tool_call, get_function_call_for_tool_execution
...
class Model(ABC):
    ...
    def _get_model_cache_file_path(self, cache_key: str) -> Path:
        ...
        if self.cache_dir:
            ...
            cache_dir = Path.home() / ".kern" / "cache" / "model_responses"
    ...
    def count_tokens(
        ...
        from kern.utils.tokens import count_tokens
        ...
        try:
            ...
            while True:
                ...
                if run_response is not None and model_response.response_usage is not None:
                    from kern.metrics import accumulate_model_metrics
            ...
            if self.__class__.__name__ == "Gemini" and self.client is not None:  # type: ignore
                try:
                    ...
                    log_warning(
                        "Your Gemini client is outdated. For Kern to properly handle the lifecycle of the client,"
            ...
            try:
                for item in function_execution_result.result:
                    ...
                    if (
                        ...
                        from kern.run.workflow import WorkflowCompletedEvent
