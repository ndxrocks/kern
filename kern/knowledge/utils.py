...
from kern.knowledge.reader.base import Reader
from kern.knowledge.reader.reader_factory import ReaderFactory
from kern.knowledge.types import ContentType
from kern.utils.log import log_debug

RESERVED_KERN_KEY = "_kern"
...
def merge_user_metadata(
    ...
    """Deep-merge two metadata dicts, preserving the ``_kern`` sub-key from both sides.

    Top-level keys from *incoming* overwrite those in *existing* (except ``_kern``).
    Keys inside ``_kern`` are merged individually so that info added
    ...
    for key, value in incoming.items():
        if key == RESERVED_KERN_KEY:
            old_kern = merged.get(RESERVED_KERN_KEY, {}) or {}
            new_kern = value if isinstance(value, dict) else {}
            merged[RESERVED_KERN_KEY] = {**old_kern, **new_kern}
        else:
            merged[key] = value
    return merged


def set_kern_metadata(
    metadata: Optional[Dict[str, Any]],
    key: str,
    value: Any,
) -> Dict[str, Any]:
    """Set a key under the reserved ``_kern`` namespace in metadata."""
    if metadata is None:
        metadata = {}
    kern_meta = metadata.get(RESERVED_KERN_KEY, {}) or {}
    kern_meta[key] = value
    metadata[RESERVED_KERN_KEY] = kern_meta
    return metadata


def get_kern_metadata(
    metadata: Optional[Dict[str, Any]],
    key: str,
) -> Any:
    """Get a key from the reserved ``_kern`` namespace in metadata."""
    if not metadata:
        return None
    kern_meta = metadata.get(RESERVED_KERN_KEY)
    if not isinstance(kern_meta, dict):
        return None
    return kern_meta.get(key)


def strip_kern_metadata(
    metadata: Optional[Dict[str, Any]],
) -> Optional[Dict[str, Any]]:
    """Return a copy of *metadata* without the reserved ``_kern`` key.
    ...
    if not metadata:
        return metadata
    return {k: v for k, v in metadata.items() if k != RESERVED_KERN_KEY}


def _get_chunker_class(strategy_type):
    """Get the chunker class for a given strategy type without instantiation."""
    from kern.knowledge.chunking.strategy import ChunkingStrategyType
    ...
    strategy_class_mapping = {
        ChunkingStrategyType.AGENTIC_CHUNKER: lambda: _import_class(
            "kern.knowledge.chunking.agentic", "AgenticChunking"
        ),
        ChunkingStrategyType.CODE_CHUNKER: lambda: _import_class("kern.knowledge.chunking.code", "CodeChunking"),
        ChunkingStrategyType.DOCUMENT_CHUNKER: lambda: _import_class(
            "kern.knowledge.chunking.document", "DocumentChunking"
        ),
        ChunkingStrategyType.RECURSIVE_CHUNKER: lambda: _import_class(
            "kern.knowledge.chunking.recursive", "RecursiveChunking"
        ),
        ChunkingStrategyType.SEMANTIC_CHUNKER: lambda: _import_class(
            "kern.knowledge.chunking.semantic", "SemanticChunking"
        ),
        ChunkingStrategyType.FIXED_SIZE_CHUNKER: lambda: _import_class(
            "kern.knowledge.chunking.fixed", "FixedSizeChunking"
        ),
        ChunkingStrategyType.ROW_CHUNKER: lambda: _import_class("kern.knowledge.chunking.row", "RowChunking"),
        ChunkingStrategyType.MARKDOWN_CHUNKER: lambda: _import_class(
            "kern.knowledge.chunking.markdown", "MarkdownChunking"
        ),
    }
