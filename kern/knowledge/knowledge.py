...
from kern.db.schemas.knowledge import KnowledgeRow
from kern.filters import EQ, FilterExpr
from kern.knowledge.content import Content, ContentAuth, ContentStatus, FileData
from kern.knowledge.document import Document
from kern.knowledge.reader import Reader, ReaderFactory
from kern.knowledge.remote_content.base import BaseStorageConfig
from kern.knowledge.remote_content.remote_content import (
    RemoteContent,
)
from kern.knowledge.remote_knowledge import RemoteKnowledge
from kern.knowledge.utils import merge_user_metadata, set_kern_metadata, strip_kern_metadata
from kern.utils.http import async_fetch_with_retry
from kern.utils.log import log_debug, log_error, log_info, log_warning
from kern.utils.string import generate_id
...
class Knowledge(RemoteKnowledge):
    ...
    def __post_init__(self):
        from kern.vectordb import VectorDb
    ...
    def insert(
        ...
        # Strip reserved _kern key from user-provided metadata
        safe_metadata = strip_kern_metadata(metadata)
            ...
            for i, filter_item in enumerate(filters):
                if not isinstance(filter_item, FilterExpr):
                    log_warning(
                        ...
                        f"AND(...), OR(...), NOT(...) from kern.filters"
                    )
                    continue
    ...
    async def _aload_from_url(
        ...
        # Store URL source metadata in _kern for source tracking
        content.metadata = set_kern_metadata(content.metadata, "source_type", "url")
        content.metadata = set_kern_metadata(content.metadata, "source_url", content.url)
    ...
    def _load_from_url(
        ...
        4. Prepare and insert the content in the vector database
        """
        from kern.utils.http import fetch_with_retry
        from kern.vectordb import VectorDb
    ...
    def _update_content(self, content: Content) -> Optional[Dict[str, Any]]:
        ...
        if self.contents_db:
            ...
            if self.vector_db:
                # Strip _kern from metadata sent to vector_db — only user fields should be searchable
                user_metadata = strip_kern_metadata(content.metadata) or {}
...
""".strip()
    ...
    def _create_search_tool(
        ...
        """
        from kern.models.message import MessageReferences
        from kern.tools.function import Function
        from kern.utils.timer import Timer
    ...
    def _create_search_tool_with_filters(
        ...
        try:
            from kern.utils.knowledge import get_agentic_or_user_search_filters
        except ImportError:
