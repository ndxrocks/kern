...
from kern.knowledge.content import Content, ContentStatus
from kern.knowledge.utils import RESERVED_KERN_KEY, strip_kern_metadata
...
class BaseLoader:
    ...
    RESERVED_METADATA_KEY = RESERVED_KERN_KEY
    ...
    def _merge_metadata(
        ...
        reserved ``_kern`` key so that user PATCH updates cannot overwrite it.
        ...
        Returns:
            Merged metadata dictionary with provider fields under ``_kern``
        """
        # Strip any user-provided _kern — this key is reserved for the framework
        merged: Dict[str, Any] = strip_kern_metadata(user_metadata) or {}
        # Store provider metadata under reserved _kern key
        merged[RESERVED_KERN_KEY] = dict(provider_metadata)
        return merged
