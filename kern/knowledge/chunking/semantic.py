...
def _get_chonkie_embedder_wrapper(embedder: Embedder):
    """Create a wrapper that adapts Kern Embedder to chonkie's BaseEmbeddings interface."""

    class _ChonkieEmbedderWrapper(BaseEmbeddings):
        """Wrapper to make Kern Embedders compatible with chonkie."""

        def __init__(self, kern_embedder: Embedder):
            super().__init__()
            self._embedder = kern_embedder
...
class SemanticChunking(ChunkingStrategy):
    ...
    Args:
        embedder: The embedder to use for generating embeddings. Can be:
            ...
            - An Kern Embedder (wrapped for chonkie compatibility)
    ...
    def _initialize_chunker(self):
        ...
        # - Kern Embedder: wrap for chonkie compatibility
