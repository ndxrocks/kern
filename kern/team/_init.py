...
from kern.eval.base import BaseEval
from kern.filters import FilterExpr
from kern.guardrails import BaseGuardrail
from kern.knowledge.protocol import KnowledgeProtocol
from kern.learn.machine import LearningMachine
...
from kern.models.utils import get_model
from kern.run.agent import RunEvent
from kern.run.team import (
    TeamRunEvent,
)
from kern.session import SessionSummaryManager, TeamSession
...
from kern.utils.safe_formatter import SafeFormatter
...
def __init__(
    ...
    from kern.utils.callables import is_callable_factory
...
def background_executor(team: "Team") -> Any:
    ...
    if team._background_executor is None:
        ...
        team._background_executor = ThreadPoolExecutor(max_workers=3, thread_name_prefix="kern-bg")
...
def _set_debug(team: "Team", debug_mode: Optional[bool] = None) -> None:
    ...
    debug_level: Literal[1, 2] = (
        cast(Literal[1, 2], int(env)) if (env := getenv("KERN_DEBUG_LEVEL")) in ("1", "2") else team.debug_level
    )
    # If the default debug mode is set, or passed on run, or via environment variable, set the debug mode to True
    if team.debug_mode or debug_mode or getenv("KERN_DEBUG", "false").lower() == "true":
...
def _set_default_model(team: "Team") -> None:
    ...
    if team.model is None:
        try:
            ...
            log_exception(e)
            log_error(
                "Kern agents use `openai` as the default model provider. Please provide a `model` or install `openai`."
            )
            exit(1)
