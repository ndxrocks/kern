...
from kern.tools import Function, Toolkit
    ...
    if knowledge_filters := kwargs.get("knowledge_filters"):
        try:
            if isinstance(knowledge_filters, str):
                ...
                from kern.filters import from_dict
...
def get_agent_by_id(
    ...
    if db and isinstance(db, BaseDb):
        from kern.agent.agent import get_agent_by_id as get_agent_by_id_db
...
def get_team_by_id(
    ...
    if db and isinstance(db, BaseDb):
        from kern.team.team import get_team_by_id as get_team_by_id_db
...
def get_workflow_by_id(
    ...
    if db and isinstance(db, BaseDb):
        from kern.workflow.workflow import get_workflow_by_id as get_workflow_by_id_db
...
def setup_tracing_for_os(db: Union[BaseDb, AsyncBaseDb, RemoteDb]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "Install with: pip install opentelemetry-api opentelemetry-sdk openinference-instrumentation-kern"
        )
    except Exception as e:
