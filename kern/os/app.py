...
from kern.db.base import AsyncBaseDb, BaseDb
...
from kern.os.router import get_base_router, get_info_router, get_websocket_router
from kern.os.routers.agents import get_agent_router
from kern.os.routers.approvals import get_approval_router
from kern.os.routers.components import get_components_router
from kern.os.routers.database import get_database_router
from kern.os.routers.evals import get_eval_router
from kern.os.routers.health import get_health_router
from kern.os.routers.home import get_home_router
from kern.os.routers.knowledge import get_knowledge_router
from kern.os.routers.memory import get_memory_router
from kern.os.routers.metrics import get_metrics_router
from kern.os.routers.registry import get_registry_router
from kern.os.routers.schedules import get_schedule_router
from kern.os.routers.session import get_session_router
from kern.os.routers.teams import get_team_router
from kern.os.routers.traces import get_traces_router
from kern.os.routers.workflows import get_workflow_router
from kern.os.settings import KernAPISettings
from kern.os.utils import (
...
from kern.remote.base import RemoteDb, RemoteKnowledge
from kern.team import RemoteTeam, Team
from kern.utils.log import log_debug, log_error, log_info, log_warning
from kern.utils.string import generate_id, generate_id_from_name
...
async def http_client_lifespan(_):
    ...
    from kern.utils.http import aclose_default_clients
...
async def scheduler_lifespan(app: FastAPI, agent_os: "AgentOS"):
    ...
    from kern.scheduler import ScheduleExecutor, SchedulePoller
...
class AgentOS:
    def __init__(
        ...
        settings: Optional[KernAPISettings] = None,
        ...
        Args:
            ...
            cors_allowed_origins: List of allowed CORS origins (will be merged with default Kern domains)
        ...
        self.settings: KernAPISettings = settings or KernAPISettings()
        ...
        if self.telemetry:
            from kern.api.os import OSLaunch, log_os_telemetry
    ...
    def resync(self, app: FastAPI) -> None:
        ...
        if self.enable_mcp_server:
            from kern.os.mcp import get_mcp_server
    ...
    def _make_app(self, lifespan: Optional[Any] = None) -> FastAPI:
        return FastAPI(
            title=self.name or "Kern AgentOS",
    ...
    def _add_jwt_middleware(self, fastapi_app: FastAPI) -> None:
        from kern.os.middleware.jwt import JWTMiddleware, JWTValidator
