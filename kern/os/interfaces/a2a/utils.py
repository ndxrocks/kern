...
from kern.run.team import TeamRunOutputEvent
...
from kern.run.workflow import StepCompletedEvent as WorkflowStepCompletedEvent
from kern.run.workflow import StepErrorEvent as WorkflowStepErrorEvent
from kern.run.workflow import StepStartedEvent as WorkflowStepStartedEvent
...
async def map_a2a_request_to_run_input(request_body: dict, stream: bool = True) -> RunInput:
    """Map A2A SendMessageRequest to Kern RunInput.
...
def _map_run_status_to_task_state(status: Optional[RunStatus]) -> TaskState:
    """Map Kern RunStatus to A2A TaskState."""
    if status is None:
...
def map_run_output_to_a2a_task(run_output: Union[RunOutput, WorkflowRunOutput]) -> Task:
    ...
    Args:
        run_output: The Kern RunOutput or WorkflowRunOutput
...
async def stream_a2a_response(
    ...
    Args:
        event_stream: The async iterator of Kern events from agent/team/workflow.arun(stream=True)
    ...
    async for event in event_stream:
        ...
        # Send content events
            ...
            message = A2AMessage(
                ...
                context_id=context_id,
                task_id=task_id,
                metadata={"kern_content_category": "content"},
            )
        ...
        # Send tool call events
            ...
            metadata: Dict[str, Any] = {"kern_event_type": "tool_call_started"}
            if event.tool:
            ...
            metadata = {"kern_event_type": "tool_call_completed"}
            if event.tool:
                metadata["tool_name"] = event.tool.tool_name or "tool"
                if hasattr(event.tool, "tool_call_id") and event.tool.tool_call_id:
                    metadata["tool_call_id"] = event.tool.tool_call_id
        ...
        # Send reasoning events
            ...
            status_event = TaskStatusUpdateEvent(
                ...
                status=TaskStatus(state=TaskState.working),
                final=False,
                metadata={"kern_event_type": "reasoning_started"},
            )
            response = SendStreamingMessageSuccessResponse(id=request_id, result=status_event)
            yield f"event: TaskStatusUpdateEvent\ndata: {json.dumps(response.model_dump(exclude_none=True))}\n\n"
            ...
            if event.reasoning_content:
                ...
                reasoning_message = A2AMessage(
                    ...
                    context_id=context_id,
                    task_id=task_id,
                    metadata={"kern_content_category": "reasoning", "kern_event_type": "reasoning_step"},
                )
            ...
            status_event = TaskStatusUpdateEvent(
                ...
                status=TaskStatus(state=TaskState.working),
                final=False,
                metadata={"kern_event_type": "reasoning_completed"},
            )
            response = SendStreamingMessageSuccessResponse(id=request_id, result=status_event)
            yield f"event: TaskStatusUpdateEvent\ndata: {json.dumps(response.model_dump(exclude_none=True))}\n\n"
        ...
        # Send memory update events
            ...
            status_event = TaskStatusUpdateEvent(
                ...
                status=TaskStatus(state=TaskState.working),
                final=False,
                metadata={"kern_event_type": "memory_update_started"},
            )
            response = SendStreamingMessageSuccessResponse(id=request_id, result=status_event)
            yield f"event: TaskStatusUpdateEvent\ndata: {json.dumps(response.model_dump(exclude_none=True))}\n\n"
            ...
            status_event = TaskStatusUpdateEvent(
                ...
                status=TaskStatus(state=TaskState.working),
                final=False,
                metadata={"kern_event_type": "memory_update_completed"},
            )
            response = SendStreamingMessageSuccessResponse(id=request_id, result=status_event)
            yield f"event: TaskStatusUpdateEvent\ndata: {json.dumps(response.model_dump(exclude_none=True))}\n\n"
        ...
        # Send workflow events
            ...
            metadata = {"kern_event_type": "workflow_step_started"}
            ...
            metadata = {"kern_event_type": "workflow_step_completed"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            ...
            metadata = {"kern_event_type": "workflow_step_error"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
        ...
        # Send loop events
            ...
            metadata = {"kern_event_type": "loop_execution_started"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            ...
            metadata = {"kern_event_type": "loop_iteration_started"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            ...
            metadata = {"kern_event_type": "loop_iteration_completed"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            if hasattr(event, "iteration") and event.iteration is not None:
                metadata["iteration"] = event.iteration
            ...
            metadata = {"kern_event_type": "loop_execution_completed"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
        ...
        # Send parallel events
            ...
            metadata = {"kern_event_type": "parallel_execution_started"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            ...
            metadata = {"kern_event_type": "parallel_execution_completed"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            if hasattr(event, "parallel_step_count") and event.parallel_step_count:
                metadata["parallel_step_count"] = event.parallel_step_count
        ...
        # Send condition events
            ...
            metadata = {"kern_event_type": "condition_execution_started"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            ...
            metadata = {"kern_event_type": "condition_execution_completed"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            if hasattr(event, "condition_result") and event.condition_result is not None:
                metadata["condition_result"] = event.condition_result
        ...
        # Send router events
            ...
            metadata = {"kern_event_type": "router_execution_started"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            ...
            metadata = {"kern_event_type": "router_execution_completed"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            if hasattr(event, "selected_steps") and event.selected_steps:
                metadata["selected_steps"] = event.selected_steps
        ...
        # Send steps events
            ...
            metadata = {"kern_event_type": "steps_execution_started"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            ...
            metadata = {"kern_event_type": "steps_execution_completed"}
            if hasattr(event, "step_name") and event.step_name:
                metadata["step_name"] = event.step_name
            if hasattr(event, "steps_count") and event.steps_count:
                metadata["steps_count"] = event.steps_count
    ...
    if cancelled_event:
        ...
        metadata = {"kern_event_type": "run_cancelled"}
