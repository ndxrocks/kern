"""Async router handling exposing an Kern Agent or Team in an A2A compatible format."""
...
from kern.os.interfaces.a2a.utils import (
    map_a2a_request_to_run_input,
    map_run_output_to_a2a_task,
    stream_a2a_response_with_error_handling,
)
from kern.os.utils import get_agent_by_id, get_request_kwargs, get_team_by_id, get_workflow_by_id
...
def attach_routes(
    ...
    @router.post(
        ...
        description="Send a message to an Kern Agent (non-streaming). The Agent is identified via the path parameter '{id}'. "
    ...
    @router.post(
        ...
        description="Stream a message to an Kern Agent (streaming). The Agent is identified via the path parameter '{id}'. "
        "Optional: Pass user ID via X-User-ID header (recommended) or 'userId' in params.message.metadata. "
    ...
    @router.post(
        ...
        description="Send a message to an Kern Team (non-streaming). The Team is identified via the path parameter '{id}'. "
        "Optional: Pass user ID via X-User-ID header (recommended) or 'userId' in params.message.metadata.",
        response_model_exclude_none=True,
        responses={
            200: {
    ...
    @router.post(
        ...
        description="Stream a message to an Kern Team (streaming). The Team is identified via the path parameter '{id}'. "
        "Optional: Pass user ID via X-User-ID header (recommended) or 'userId' in params.message.metadata. "
        "Returns real-time updates as newline-delimited JSON (NDJSON).",
        response_model_exclude_none=True,
        responses={
    ...
    @router.post(
        ...
        description="Send a message to an Kern Workflow (non-streaming). The Workflow is identified via the path parameter '{id}'. "
        "Optional: Pass user ID via X-User-ID header (recommended) or 'userId' in params.message.metadata.",
        response_model_exclude_none=True,
        responses={
            200: {
    ...
    @router.post(
        ...
        description="Stream a message to an Kern Workflow (streaming). The Workflow is identified via the path parameter '{id}'. "
        "Optional: Pass user ID via X-User-ID header (recommended) or 'userId' in params.message.metadata. "
        "Returns real-time updates as newline-delimited JSON (NDJSON).",
        response_model_exclude_none=True,
        responses={
    ...
    @router.post(
        ...
        name="send_message",
        description="[DEPRECATED] Send a message to an Kern Agent, Team, or Workflow. "
    ...
    @router.post(
        ...
        name="stream_message",
        description="[DEPRECATED] Stream a message to an Kern Agent, Team, or Workflow. "
        "The Agent, Team or Workflow is identified via the 'agentId' field in params.message or X-Agent-ID header. "
        "Optional: Pass user ID via X-User-ID header (recommended) or 'userId' in params.message.metadata. "
        "Returns real-time updates as newline-delimited JSON (NDJSON).",
        response_model_exclude_none=True,
