...
from kern.run.agent import RunContentEvent, RunEvent, RunOutputEvent, RunPausedEvent
from kern.run.team import RunContentEvent as TeamRunContentEvent
from kern.run.team import TeamRunEvent, TeamRunOutputEvent
from kern.utils.log import log_debug, log_warning
from kern.utils.message import get_text_from_message
...
@dataclass
class EventBuffer:
    """Buffer to manage event ordering constraints, relevant when mapping Kern responses to AG-UI events."""
...
def convert_agui_messages_to_kern_messages(messages: List[AGUIMessage]) -> List[Message]:
    """Convert AG-UI messages to Kern messages."""
...
def _create_events_from_chunk(
    ...
    # Handle custom events
        ...
        # Use the complete Kern event as value if parsing it works, else the event content field
        try:
...
def stream_kern_response_as_agui_events(
    response_stream: Iterator[Union[RunOutputEvent, TeamRunOutputEvent]], thread_id: str, run_id: str
) -> Iterator[BaseEvent]:
    """Map the Kern response stream to AG-UI format, handling event ordering constraints."""
    ...
    if not stream_completed:
        ...
        from kern.run.agent import RunCompletedEvent
...
async def async_stream_kern_response_as_agui_events(
