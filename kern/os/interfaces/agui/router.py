"""Async router handling exposing an Kern Agent or Team in an AG-UI compatible format."""
...
from kern.os.interfaces.agui.utils import (
    async_stream_kern_response_as_agui_events,
    convert_agui_messages_to_kern_messages,
    validate_agui_state,
)
...
async def run_agent(agent: Union[Agent, RemoteAgent], run_input: RunAgentInput) -> AsyncIterator[BaseEvent]:
    """Run the contextual Agent, mapping AG-UI input messages to Kern format, and streaming the response in AG-UI format."""
    run_id = run_input.run_id or str(uuid.uuid4())

    try:
        # Preparing the input for the Agent and emitting the run started event
        messages = convert_agui_messages_to_kern_messages(run_input.messages or [])
        ...
        async for event in async_stream_kern_response_as_agui_events(
...
async def run_team(team: Union[Team, RemoteTeam], input: RunAgentInput) -> AsyncIterator[BaseEvent]:
    """Run the contextual Team, mapping AG-UI input messages to Kern format, and streaming the response in AG-UI format."""
    run_id = input.run_id or str(uuid.uuid4())
    try:
        # Extract the last user message for team execution
        messages = convert_agui_messages_to_kern_messages(input.messages or [])
