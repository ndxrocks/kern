...
from kern.os.utils import get_db
...
def get_memory_router(
    dbs: dict[str, list[Union[BaseDb, AsyncBaseDb, RemoteDb]]], settings: KernAPISettings = KernAPISettings(), **kwargs
) -> APIRouter:
...
def attach_routes(router: APIRouter, dbs: dict[str, list[Union[BaseDb, AsyncBaseDb, RemoteDb]]]) -> APIRouter:
    ...
    async def optimize_memories(
        ...
        from kern.memory.strategies.types import MemoryOptimizationStrategyType
