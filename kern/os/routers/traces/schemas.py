...
from kern.os.utils import format_duration_ms
...
def _derive_span_type(span: Any) -> str:
    ...
    We use additional context (kern.team.id, kern.workflow.id) to differentiate:
    - WORKFLOW: CHAIN spans or spans with kern.workflow.id
    - TEAM: AGENT spans with kern.team.id
    - AGENT: AGENT spans without kern.team.id
    ...
    if span_kind == "AGENT":
        ...
        if span.attributes.get("kern.team.id") or span.attributes.get("team.id"):
            return "TEAM"
        return "AGENT"
...
class TraceNode(BaseModel):
    ...
    def from_span(cls, span: Any, spans: Optional[List["TraceNode"]] = None) -> "TraceNode":
        ...
        if span_kind == "AGENT":
            if run_id := span.attributes.get("kern.run.id"):
            ...
            if workflow_description := span.attributes.get("kern.workflow.description"):
                metadata["description"] = workflow_description
            if steps_count := span.attributes.get("kern.workflow.steps_count"):
                metadata["steps_count"] = steps_count
            if steps := span.attributes.get("kern.workflow.steps"):
                metadata["steps"] = steps
            if step_types := span.attributes.get("kern.workflow.step_types"):
...
class TraceDetail(BaseModel):
    ...
    def _build_span_tree(
        ...
        for root_span in root_spans:
            span_kind = root_span.attributes.get("openinference.span.kind", "")
            if span_kind == "CHAIN":
                step_types = root_span.attributes.get("kern.workflow.step_types", [])
                if step_types:
