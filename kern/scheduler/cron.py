...
def _require_croniter() -> None:
    if croniter is None:
        raise ImportError("`croniter` not installed. Please install it using `pip install kern[scheduler]`")


def _require_pytz() -> None:
    if pytz is None:
        raise ImportError("`pytz` not installed. Please install it using `pip install kern[scheduler]`")
