...
class KernLogger(logging.Logger):
...
def build_logger(logger_name: str, source_type: Optional[str] = None) -> Any:
    # If a logger with the name "kern.{source_type}" is already set, we want to use that one
    _logger = logging.getLogger(f"kern.{logger_name}")
    if _logger.handlers or _logger.level != logging.NOTSET:
        return _logger

    # Set the custom logger class as the default for this logger
    logging.setLoggerClass(KernLogger)
    ...
    rich_handler = ColoredRichHandler(
        ...
        show_path=True if getenv("KERN_API_RUNTIME") == "dev" else False,
...
agent_logger: KernLogger = build_logger(LOGGER_NAME, source_type="agent")
team_logger: KernLogger = build_logger(TEAM_LOGGER_NAME, source_type="team")
workflow_logger: KernLogger = build_logger(WORKFLOW_LOGGER_NAME, source_type="workflow")

# Set the default logger to the agent logger
logger: KernLogger = agent_logger
...
def _using_default_logger(logger_instance: Any) -> bool:
    """Return True if the currently active logger is our default KernLogger"""
    return isinstance(logger_instance, KernLogger)
...
def configure_kern_logging(
    ...
    """
    Util to set custom loggers. These will be used everywhere across the Kern library.
