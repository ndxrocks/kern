...
from kern.run.agent import RunEvent, RunOutput, run_output_event_from_dict
from kern.run.base import BaseRunOutputEvent, RunStatus
from kern.run.team import TeamRunEvent, TeamRunOutput, team_run_output_event_from_dict
...
class WorkflowRunOutput:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WorkflowRunOutput":
        ...
        from kern.workflow.step import StepOutput

        workflow_metrics_dict = data.pop("metrics", {})
        workflow_metrics = None
        if workflow_metrics_dict:
            from kern.workflow.workflow import WorkflowMetrics
        ...
        for event in events_data or []:
            if "agent_id" in event:
                ...
                from kern.run.agent import run_output_event_from_dict

                event = run_output_event_from_dict(event)
            elif "team_id" in event:
                # Team event from team step
                from kern.run.team import team_run_output_event_from_dict
        ...
        if step_requirements_data:
            from kern.workflow.types import StepRequirement
        ...
        if router_requirements_data:
            from kern.workflow.types import StepRequirement as StepReq
        ...
        if error_requirements_data:
            from kern.workflow.types import ErrorRequirement
