"""
Trace data models for Kern tracing.
"""
...
def create_trace_from_spans(spans: List[Span]) -> Optional[Trace]:
    ...
    run_id = attrs.get("run_id") or attrs.get("kern.run.id")

    session_id = attrs.get("session_id") or attrs.get("kern.session.id") or attrs.get("session.id")

    user_id = attrs.get("user_id") or attrs.get("kern.user.id") or attrs.get("user.id")

    # Try to extract agent_id from the span name or attributes
    agent_id = attrs.get("agent_id") or attrs.get("kern.agent.id")

    team_id = attrs.get("team_id") or attrs.get("kern.team.id")

    workflow_id = attrs.get("workflow_id") or attrs.get("kern.workflow.id")
