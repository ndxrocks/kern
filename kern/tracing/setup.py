"""
Setup helper functions for configuring Kern tracing.
"""

from typing import Union

from kern.db.base import AsyncBaseDb, BaseDb
from kern.remote.base import RemoteDb
from kern.tracing.exporter import DatabaseSpanExporter
from kern.utils.log import logger

try:
    from openinference.instrumentation.kern import KernInstrumentor  # type: ignore
...
def setup_tracing(
    ...
    """
    Set up OpenTelemetry tracing with database export for Kern agents.

    This function configures automatic tracing for all Kern agents, teams, and workflows.
    ...
    try:
        ...
        # Instrument Kern with OpenInference
        KernInstrumentor().instrument(tracer_provider=tracer_provider)

        logger.info("Kern tracing successfully set up with database storage")
